...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(f"{BASE_URL}/auth/signup", json={"email": bad_doc_email, "password": "password123", "name": "Bad Doc", "role": "THERAPIST", "phone":"000"}) as resp:
            print(f"Bad Doc Signup Status: {resp.status}")
            if resp.status not in [201, 400]:
                 print(f"Bad Doc Signup Error: {await resp.text()}")
        await asyncio.to_thread(users_collection.update_one, {"email": bad_doc_email}, {"$set": {"is_verified": True}})
        async with session.post(f"{BASE_URL}/auth/login", json={"email": bad_doc_email, "password": "password123"}) as resp:
            print(f"Bad Doc Login Status: {resp.status}")
            if resp.status != 200:
                print(f"Bad Doc Login Error: {await resp.text()}")
